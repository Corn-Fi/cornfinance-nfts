...
const mint = async () => {
    ...
    const tx = await NFTContract.mint({gasPrice: ethers.utils.parseUnits('35', 'gwei'), gasLimit: 1000000});
    // console.log(`approved Controller to spend your ${tokenAddress}`)
    const receipt = await tx.wait()
    return receipt
};

const deploy = async (name, symbol, maxSupply, URI, feeToken, treasuryAddr, devAddr, treasuryFee, devFee) => {
    const signer = await fetchSigner();
    const NFT = await hre.ethers.getContractFactory("CFNFT");
    const nft = await NFT.deploy(name, symbol, maxSupply, URI, feeToken, treasuryAddr, devAddr, treasuryFee, devFee);
    return nft;
}
...
async function main() {
  ...
  // await approveToken("0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270", "0x61cCe285013395AB905Bd9f43757922D000debbE");
